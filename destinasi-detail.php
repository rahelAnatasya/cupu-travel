...
// Fetch images for the selected destination
$images = [];
$sql = "SELECT gambar_url FROM destinasi_gambar WHERE destinasi_id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $_GET['id']);
$stmt->execute();
$result_images = $stmt->get_result();
while ($row = $result_images->fetch_assoc()) {
    $images[] = $row['gambar_url'];
}
$stmt->close();
$conn->close();
?>
...
<body>
            ...
            <?php if (!empty($searchResults)):
                ...
                <div class="col-12 col-md-6 d-flex flex-column align-items-center mb-4">
                    <div class="rounded-4 overflow-hidden mb-3 w-100" style="max-height: 300px;">
                        <img src="<?php echo $result['thumbnail']; ?>" alt="<?php echo $result['nama_tempat']; ?>"
                ...
                <div class="col-12 col-md-6">
                    ...
                    <table>
                        ...
                        <tr>
                            <td style="width: 150px;">Jam Buka</td>
                            <td>:
                                <?= $result['jam_buka'] ?>
                            </td>
                        </tr>
                    ...
                    <div class="d-flex justify-content-center mt-4">
                        <a href="pesan.php?id=<?= $result['id'] ?>" class="btn btn-outline-dark fw-bold">Pesan Tiket</a>
                    </div>
                </div>
            <?php else: ?>
                <p class="text-center">Destinasi ini tidak ditemukan "<?php echo htmlspecialchars($query); ?>"
                </p>
            <?php endif; ?>
        </div>
    </div>

    <!-- New section for destination images -->
    <?php
    if (count($images) > 0):
        ?>
        <div class="container">
            <h3 class="mb-5">Gambar Destinasi</h3>
            <div class="row mb-5">
                <?php foreach ($images as $image): ?>
                    <div class="col-12 col-md-4 col-xl-4 d-flex flex-column align-items-center mb-4">
                        <div class="rounded-4 overflow-hidden mb-3 h-100 w-100" style="max-height: 200px;">
                            <img src="<?php echo $image; ?>" alt="Gambar Destinasi" class="img-fluid h-100"
    ...
    /*
    <div class="container">
        <h3 class="mb-5">Kamu mungkin juga suka</h3>
        <div class="row mb-5">
            <?php if (!empty($searchResults)): ?>
                <?php
                $count = 0;
                foreach ($searchResults as $result):
                    if ($result['id'] != $_GET['id'] && $count < 9):
                        $count++;
                        ?>
                        <a href="destinasi-detail.php?id=<?= $result['id'] ?>"
                            class="col-12 col-md-4 col-xl-4 d-flex flex-column align-items-center mb-4 text-decoration-none text-black">
                            <div class="rounded-4 h-100 overflow-hidden mb-3 w-100" style="max-height: 200px;">
                                <img src="<?php echo $result['thumbnail']; ?>" alt="<?php echo $result['nama_tempat']; ?>"
                                    class="img-fluid" style="object-fit: cover; width: 100%; height: 100%;">
                            </div>
            ...
            <?php else: ?>
                <p class="text-center">Tidak ada hasil ditemukan untuk pencarian "<?php echo htmlspecialchars($query); ?>"
                </p>
            <?php endif; ?>
        </div>
    </div>
            ...
            <div class="px-5">
                <div class="d-flex">
                    <svg width="26" height="27" viewBox="0 0 26 27" fill="none" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <rect x="0.834061" y="0.589844" width="25" height="25"
                            transform="rotate(1.91188 0.834061 0.589844)" fill="url(#pattern0_23_153)" />
                        <defs>
