...
if (isset($_GET['query'])) {
    $query = $_GET['query'];
    $sql = "SELECT * FROM destinasi WHERE nama_tempat LIKE ?";
...
<?php include "navbar.php" ?>
    ...
    <div class="container fs-4 d-flex align-items-center mt-5 mb-4">
        <a href="<?= isset($_GET['query']) ? "cari.php" : "index.php"?>">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                class="size-6 me-2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18" />
            </svg>
        </a>
        Cari Tempat Rekreasi mu
    </div>

    <form class="container mb-5" action="cari.php" method="GET">
        <div class="input-group">
            <input type="text" class="form-control" name="query" placeholder="Cari tempat rekreasimu, disini" required>
        ...
        <div class="row mb-5">
            <?php if (!empty($searchResults)): ?>
                <?php foreach ($searchResults as $result): ?>
                    <a href="destinasi-detail.php?id=<?= $result['id'] ?>" class="col-12 col-md-6 col-xl-4 d-flex flex-column align-items-center mb-4 text-decoration-none text-black">
                        <div class="rounded-4 overflow-hidden mb-3 w-100" style="max-height: 200px;">
                            <img src="<?php echo $result['thumbnail']; ?>" alt="<?php echo $result['nama_tempat']; ?>" class="img-fluid" style="object-fit: cover; width: 100%; height: 100%;">
                        </div>
                        <div class="fs-5">
                            <?php echo $result['nama_tempat']; ?>
                        </div>
                        <div class="fw-bold">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p class="text-center">Tidak ada hasil ditemukan untuk pencarian "<?php echo htmlspecialchars($query); ?>"</p>
            <?php endif; ?>
        </div>
    </div>
